...
### 6.7.3 著名的“非官方解密”风波（PRCoords）

GCJ-02 算法的源码由官方控制，且被视为国家机密。然而，随着智能手机普及和逆向工程的发展，由于该算法必须在数以亿计的移动端设备或浏览器前端进行加解密运算，其算法逻辑不可避免地暴露在了内存和前端代码中。

- **非线性解密**：大约在 2010 年前后，匿名开发者通过逆向工程提取出了这段加密代码（包含大量基于克拉索夫斯基椭球参数的魔术数字和正弦交叉变换），并开源在 GitHub 等平台上，史称 **PRCoords**（Public Release Coordinates）。
- **完全可逆与反演**：数学家和程序员证明，虽然 GCJ-02 加密函数是单向且包含高频震荡项，但在实际空间域中，它是一个平滑的连续函数。利用牛顿迭代法或二分法，开发者可以实现精度高达厘米级的完美逆变换（WGS84 ⇌ GCJ-02 互转），使这种“加密”在技术极客面前形同虚设。
