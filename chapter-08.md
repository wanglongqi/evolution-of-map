...
由于失真通常具有方向依赖性，需要考虑极值：

$$\text{最大长度比: } k_{max} = \max(k, h, \sqrt{kh})$$

$$\text{最小长度比: } k_{min} = \min(k, h, \sqrt{kh})$$
...
$$a = \frac{\sqrt{h^2 + k^2 + 2hk\cos\theta'}}{2}$$

$$b = \frac{\sqrt{h^2 + k^2 - 2hk\cos\theta'}}{2}$$
