...
$$
f(u) = rac{1}{2} \left[ rac{	an(u \cdot rac{\pi}{4})}{	an(rac{\pi}{4})} + 1 \right]
$$
...
其最大的数论优越性在于：任意一维哈希值接近的两个 Cell，其在二维曲面空间上也绝对邻近。将百万级图元求交集，在此映射下被转化为求几组 64位无符号整数的区间联合（Interval Union, $^Higcup_{i} [start_i, end_i]$ ）。
