...
1. 初始化经纬度区间。
2. 对于纬度区间 $[a, b]$：
   若 $\phi_0 > \frac{a+b}{2}$，提取位 `1`，并将区间更新为 $[\frac{a+b}{2}, b]$。
   若 $\phi_0 \le \frac{a+b}{2}$，提取位 `0`，并将区间更新为 $[a, \frac{a+b}{2}]$。
...
这里需要厘清一个空间拓扑学上的反直觉现象：
**任意一维哈希值接近的两个点，其在二维曲面空间上也绝对邻近。**（由于曲线的连续性）。但这并非双向的，**空间上相邻的两个网格，其一维哈希值有时会相差极大**（即由于降维不可避免导致的空间阶跃突变）。
